...
void experiment(const int a, const int b, const int c, const int d, const int e, const std::string& filename)
{
	...
	{
		MemeticAlgorithm ma(100, 0.3, 0.1, filename);
		result.push_back(ma.run(a, b, c, d, e)); //2, 4, 3, 3, 2
		// default: 1, 0, 0, 0, 0
	}
...
int main() 
{
	...
	for (const std::string i : dataset)
	{
		...
		{
			experiment(1, b, 0, 0, 0, i);
		}
		for (std::size_t c = 0; c < 3; c += 1)
		{
			experiment(1, 0, c, 0, 0, i);
		}
		for (std::size_t d = 0; d < 3; d += 1)
		{
			experiment(1, 0, 0, d, 0, i);
		}
		for (std::size_t e = 0; e < 2; e += 1)
		{
